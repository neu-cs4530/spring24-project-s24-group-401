  ...
  describe('applyMove', () => {
    ...
    describe('[T2.2] when given an invalid move', () => {
      ...
      describe('when the game is in progress', () => {
        ...
        it('should not change whose turn it is when an invalid move is made', () => {
          ...
          expect(game.state.word).toHaveLength(1);
          game.applyMove({
            gameID: game.id,
            playerID: player2.id,
            move: {
          ...
          expect(game.state.word).toHaveLength(2);
        });
    ...
    describe('when given a valid move', () => {
      ...
      function makeMoveAndCheckState(
        ...
        expect(game.state.word).toHaveLength(++numMoves);
        for (let i = 0; i < numMoves; i++) {
          expect(game.state.word[i]).toEqual(moves[i]);
        }
